...
def upload_image(file, toy_id):
    print(toy_id)
    if toy_id != '0' and file and allowed_file(file.filename):
        filename = secure_filename(file.filename)
        filename = 'user' + str(session['user_id']) + '-' +'toy-id'+ str(toy_id) + '-' + filename
...
def delete_old_image_from_folder(toy_id):
    image_to_delete = productsCrudDbManager.get_image_to_delete(toy_id)
    image_to_delete = str(image_to_delete)

    if image_to_delete  !='None' and image_to_delete != '':
...
def update_product(page_state):
    ...
    if file.filename == '':
        ...
        toy_image = upload_image(file, toy_id)
    ...
    if check_validation:
        ...
        delete_old_image_from_folder(toy_id)

    productsCrudDbManager.update_product(toy_id, toy_name, toy_category, toy_condition, toy_image)
